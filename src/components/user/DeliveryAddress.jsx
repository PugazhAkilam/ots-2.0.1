...
//  console.log("delete");
 ...
 // alert(`delete ${addressId}`)
  ...
  //const addressId = addresses[selectedAddress].AddressId;
    ...
    // const message = `Order placed successfully! Cart Items: ${cartItems.map(item => item.servSubType_Name).join(', ')}\nTotal Price: ${totalPrice}\nDelivery Address ID: ${addressId}\nPayment Method: ${paymentMethod}\nTotal Quantity: ${totalquantity}`;
    //     // Prepare the message content
    //     for (const number of phoneNumbers) {
    //       const messageUrl = `https://wav5.algotechnosoft.com/api/send?type=text&number=${number}&message=${encodeURIComponent(message)}&access_token=6467076147e12&instance_id=6718CDB642A77`;
    //       await axios.get(messageUrl); // Sending the message
    //     }
    
    
      
    
        // Remove items from the cart based on the service type
        cartItems.forEach(item => {
          dispatch({ type: 'REMOVE_FROM_CART', payload: { id: item.servSubType_Id } });
        });
    
        // Clear local storage related to cart
        ...
        displayedAddresses.map((address, index) => (
          <Card sx={{ marginBottom: 2, position: 'relative' }} key={index}>
            <CardActionArea
              ...
              sx={{
                backgroundColor: selectedAddress === index ? 'rgb(100, 100, 100)' : 'transparent',
                color: selectedAddress === index? 'white' : 'black',
                borderRadius: '4px',
